...
function codegen(objs) { // objs: [obj]
    var code = ""
    // Add class and constructor to code
    code += "class " + CLASS_NAME + " extends Phaser.Scene {\n" +
        "constructor() {\n" +
        "super({ key: '" + CLASS_NAME + "' })\n" +
        "}\n\n"
...
function preload(paths) { // Makes preload section
    var tcode = ""
    // Add preload itself
    tcode += "preload() {\n" +
...
function create(attributes) { // Makes create section
    var tcode = ""
    // Add create itself
    tcode += "create() {\n" +
        "// Bellow added by p3ve\n"
    ...
    for (const e of attributes) {
        tcode += "this.add.image(" + e.width + ", " + e.height + ", " + e.fname + ")\n" +
            ".setAngle(" + e.angle + ")\n" +
            ".setDepth(" + e.z + ")\n" +
            ".setPosition(" + e.left + ", " + e.top + ")\n" +
            ".setFlip(" + e.flipX + ", " + e.flipY + ")\n" +
            ".setName(" + e.name + ")\n" +
